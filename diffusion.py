...
class Attention(nn.Module):
    def __init__(self, num_chans, num_heads, dropout):
        ...
        self.proj1 = nn.Linear(num_chans, num_chans * 3)
        self.proj2 = nn.Linear(num_chans, num_chans)
...
def train(batch_size=64, steps=1000, epochs=15, ema_decay=0.9999, lr=2e-5, checkpoint_path=None):
    trainset = datasets.MNIST(root='./data', train=True, download=False,
                              transform=transforms.ToTensor())
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True,
                                               num_workers=2)

    scheduler = DDPM_Scheduler(T=steps)
    model = UNET()
    ...
    for i in range(epochs):
        total_loss = 0
        for bidx, (x,_) in enumerate(tqdm(train_loader, desc=f"Epoch {i+1}/{epochs}")):
            x = x
            x = F.pad(x, (2,2,2,2))
...
def inference(checkpoint_path=None, num_time_steps=1000, ema_decay=0.9999):
    checkpoint = torch.load(checkpoint_path)
    model = UNET()
    model.load_state_dict(checkpoint['weights'])
    ema = ModelEmaV3(model, decay=ema_decay)
    ema.load_state_dict(checkpoint['ema'])
    scheduler = DDPM_Scheduler(T=num_time_steps)
...
if __name__ == '__main__':
    ...
    # plt.imshow(diffused)
    # plt.show()
    train(checkpoint_path=None, lr=2e-5, epochs=75)
